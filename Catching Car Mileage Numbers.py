def num_interesting(number):
    ...
    if len(set(number)) == 1:   #set задает множество , и если в нем будут повторяющиеся числа то их длина будет равна 1
        print("Has same digits") #например set([2,2,2,2,2] даст {2} а его длина будет 1)
        return True

    # Dirty check for palindrome
    if number[::-1] == number:# конструкция [::-1] переворачивает массив задом наперед
        print("Palindrome")
        return True
